-- Root cause: set_order_number BEFORE INSERT trigger on orders overwrites order_number.
-- purchase_with_wallet was generating a number, inserting it, but the trigger replaced it
-- with a different number. Function then returned the pre-generated (wrong) number.
-- Fix: use RETURNING id, order_number to get the ACTUAL stored order number.
...
BEGIN
  ...
  -- Insert order. The set_order_number BEFORE INSERT trigger will overwrite order_number,
  -- so we use RETURNING id, order_number to get the ACTUAL stored order number.
