-- Fix double-assignment: purchase_with_wallet was assigning credentials AND
-- trg_auto_assign_credentials_insert (AFTER INSERT on orders) was ALSO assigning.
-- Result: 2 credentials for a quantity-1 order.
--
-- Solution: remove explicit assignment from purchase_with_wallet.
-- The INSERT trigger handles assignment. The email trigger fires when order → completed.
...
BEGIN
  ...
  -- Create order — trg_auto_assign_credentials_insert fires here and assigns credentials
